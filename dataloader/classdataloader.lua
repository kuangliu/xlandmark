...
--  - dataPath: a folder containing the images organized in subfolders.
--              the subfolder name is the class name.
--  - imfunc: the image processing function
--
function ClassDataLoader:__init(dataPath, imfunc)
    assert(paths.dirp(dataPath), dataPath..' not exist!')
    self.dataPath = dataPath
    self.__imfunc = imfunc or function(im) return im end
...
function ClassDataLoader:__getClasses()
    self.classes = {}
    local dirs = dir.getdirectories(self.dataPath)
...
function ClassDataLoader:__getFileNames()
    ...
    for k,v in pairs(self.classes) do
        print('parsing '..v..'..')
        local classPath = pathcat(self.dataPath, v)
...
function ClassDataLoader:__loopfolder(path)
    ...
    self.nSamples = (self.nSamples or 0) + N
...
function ClassDataLoader:sample(quantity)
    ...
    for i = 1,quantity do
        ...
        samples = samples or torch.Tensor(quantity, 3, im:size(2), im:size(2))
        samples[i] = im
        targets[i] = class
    end
...
function ClassDataLoader:__loadSampleByClass(class)
    ...
    local im = image.load(pathcat(self.dataPath, self.classes[class], name))
    return self.__imfunc(im)
end
...
function ClassDataLoader:get(i1,i2)
    ...
    for i = i1,i2 do
        ...
        local im = image.load(pathcat(self.dataPath, self.classes[classidx], name))
        im = self.__imfunc(im)
